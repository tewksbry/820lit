...
//  820lit
//
//  Created by Stiven Deleur on 1/31/17.
...
class HSBColorPicker : UIView {
    ...
    func touchedColor(_ gesture: UILongPressGestureRecognizer){
        let point = gesture.location(in: self)
        let color = getColorAtPoint(point: point)
        
        self.delegate?.HSBColorColorPickerTouched(sender: self, color: color, point: point, state:gesture.state)
    }
}
